  ...
  createCache = async () => {
    try {
      ...
      if(JSON.parse(currentMonth) === monthValue) {
        ...
        // delete previous month records
        date.setDate(1)
        date.setMonth(monthValue - 1)
        date = JSON.stringify(date)
        date = currentDate.substring(1,11)
        let tIdList = []
        db.transaction((tx) => {
          tx.executeSql('select record_title_id as tId from record_title_data where date(createdOn) < \'' + date + '\'', [], (tx, data) => {
            // alert(data.rows.length)
            for (let index = 0; index < data.rows.length; index++) {
              tIdList.push(data.rows.item(index))
            }
            deleteData(tIdList)
          }, (e) => {
              // console.log('error' + JSON.stringify(e)) 
    ...
    try {
      ...
      if(data) {
        for(i = 0; i < data.length; i++) {
          ...
          datevalue = new Date(data[i].contentDate)
        ...
        <View style={{ flex: 1, marginLeft: this.state.orientation === 'PORTRAIT' ? null : 30 }}>
          <View style={{backgroundColor: this.state.orientation === 'PORTRAIT' ? this.props.footer : null}}>
            <Text style={[styles.storiesText, { alignSelf: this.state.orientation === 'PORTRAIT' ? 'center' : null, marginLeft: this.state.orientation === 'PORTRAIT' ? null : 7}]}>
              Stories on {moment(this.state.selectedDate).format('MMM DD, YYYY')}
            </Text>
          </View>
            ...
            renderRow={(rowData) => {
              ...
              <View style={{borderBottomColor: 'gray', borderBottomWidth: 0.5}}>
                <TouchableHighlight
                  ...
                  onPress={() => {                  
                    this.props.dispatch(dateChange(new Date(this.state.selectedDate)))
